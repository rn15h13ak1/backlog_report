...
等式 `残件数 + 新規発生 = 完了 + 未完了` が成立しない場合は差し戻し警告をサマリー直下に表示。

**各カテゴリのセクション（4 つ）**

それぞれのセクションに以下の 2 階層で出力されます。

1. **期間開始前からの残件** — 課題番号のコンパクト一覧（最大 20 件）＋ 詳細テーブル（折りたたみ）
2. **新規発生** — 同上
3. **期間内完了** — 同上
4. **現在の未完了一覧** — 同上（詳細テーブルは最大 50 件）
...
詳細テーブルは `<details>` タグで折りたたまれており、GitHub や対応 Markdown ビューアで展開/折りたたみができます。
